...
- `operations` module — the shared refactoring layer over `liwe`'s operation primitives: discovering and selecting a document's sections or block references by title or 1-based number (`sections`, `select_section`, `references`, `select_reference`, `SelectError`), and `attach_reference`, which builds a reference into a target and reports whether to update it, create it, or skip an already-attached target.
