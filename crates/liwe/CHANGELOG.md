...
- `MarkdownOptions.shorten_wiki_links: bool` (default `true`) controls whether wiki links are rewritten to their shortest unambiguous path suffix; when `false`, `KeyIndex::shorten_wiki` returns the full key path. `KeyIndex` gains `set_shortening(&mut self, bool)` to carry the setting, fed from the markdown options when the index is built.
