...
### Changed
- Workspace-symbol search blends BM25 full-text relevance over document title and body with the existing fuzzy match (previously fuzzy only), so a query term in a document's body can lift it above an equally-fuzzy result.
